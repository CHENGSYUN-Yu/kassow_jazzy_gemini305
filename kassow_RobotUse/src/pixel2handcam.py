"""
pixel2handcam.py — 手腕相機（Orbbec Gemini 305）像素座標轉相機座標系物件（獨立工廠）

職責：
    將手腕相機（Orbbec Gemini 305）的器械重心像素 (u, v) 與深度值，
    利用 Gemini 305 相機內參反投影，轉換為手腕相機座標系下的 3D 位置（mm）。

    功能與 Pixel2HeadCam 相同，但完整獨立實作，相機內參對應 Gemini 305：
    - 相機內參由外部從 Orbbec SDK 取得後設定
    - 可針對 Gemini 305 特性獨立調整，不影響頭部相機的座標轉換

座標系說明：
    手腕相機座標系（Gemini 305 camera frame）：
        X 軸：向右
        Y 軸：向下
        Z 軸：朝向物體（光軸方向）
    單位：mm
...
外部使用方式：
    p2h = Pixel2HandCam()
    p2h.set_intrinsics(fx, fy, cx, cy)   # 從 Orbbec SDK 取得 (848×530 模式)
...
class Pixel2HandCam:
    """
    手腕相機（Orbbec Gemini 305）像素座標轉相機座標系物件。

    相機內參（Gemini 305）由外部設定，轉換邏輯封裝在內部。
    ...
    def set_intrinsics(self, fx: float, fy: float,
                       cx: float, cy: float):
        """
        設定 Gemini 305 相機內參（來自 Orbbec SDK）。

        參數：
            fx, fy : 焦距（像素單位）
                   當前值: fx=409.33, fy=409.11 (848×530 模式)
            cx, cy : 主點（光軸在影像上的位置，像素單位）
                   當前值: cx=422.81, cy=272.61 (848×530 模式)
        """
        self._fx = float(fx)
        self._fy = float(fy)
        self._cx = float(cx)
