...
職責：
    ...
    將手腕相機（Orbbec Gemini 305）座標系下的 3D 位置，
    透過 T_matrix_handcam2flange 轉換為法蘭（flange）座標系（mm）。

    功能與 HeadCam2Base 相同，但完整獨立實作：
    - T_matrix 對應 EIH 校正結果（T_cam2gripper_gemini.npy）
...
外部使用方式：
    h2f = HandCam2Flange()
    h2f.load_T('T_cam2gripper_gemini.npy')
...
class HandCam2Flange:
    ...
    def load_T(self, path: str):
        """從 .npy 檔載入 T_handcam2flange（對應 Gemini 305 EIH 校正的 T_cam2gripper_gemini.npy）。"""
